...
Now, for the complex number case of $\zeta(s)$, we would have $s=a+bi$ where $re(s)=a$ and $im(s)=b$, then the sum diverges for ${0}<{re(s)}<{1}$ (same as the real number case) and wil have a <strong>'simple pole (singularity)'</strong> at $re(s)=1$ for which $1/zeta(s)=1/0$.
